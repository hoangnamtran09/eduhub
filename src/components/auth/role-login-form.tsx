...
export function RoleLoginForm({ role, title, description, showGoogleLogin = false }: RoleLoginFormProps) {
  return (
    <Suspense fallback={null}>
      <RoleLoginFormContent
        role={role}
        title={title}
...
function RoleLoginFormContent({ role, title, description, showGoogleLogin }: RoleLoginFormProps) {
  ...
  const roleChooserHref = safeCallbackUrl ? `/login?callbackUrl=${encodeURIComponent(safeCallbackUrl)}` : "/login";
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12 bg-paper-100">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            <Link href={roleChooserHref} className="text-sm text-brand-600 hover:text-brand-700 hover:underline">
              Chọn vai trò khác
            </Link>
            <h2 className="font-serif text-3xl font-bold text-ink-900 mt-4 mb-2">{title}</h2>

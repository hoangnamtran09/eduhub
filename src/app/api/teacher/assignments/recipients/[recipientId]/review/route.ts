...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const recipient = await prisma.assignmentRecipient.findUnique({
      ...
      include: {
        assignment: true,
        student: { select: { teacherId: true } as any },
      },
    });
    ...
    if (
      ...
      || (recipient.student as any)?.teacherId !== authorization.authUser.userId
    ) {
    ...
    const updated = await prisma.assignmentRecipient.update({
      ...
      include: {
        student: { select: { id: true, fullName: true, email: true, gradeLevel: true } },
    ...
    console.error("Review teacher assignment submission error:", error);

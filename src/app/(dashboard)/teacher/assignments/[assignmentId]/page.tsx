...
export default function TeacherAssignmentDetailPage() {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <div>
        <Link href="/teacher/assignments" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-900">
          <ArrowLeft className="h-4 w-4" />
          Quay lại danh sách bài tập
        </Link>
        <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Chi tiết bài tập</h1>
        <p className="mt-2 text-sm text-slate-500">Theo dõi trạng thái nộp bài và chấm bài cho học sinh của bạn.</p>
      </div>
      ...
      <AssignmentCard assignment={assignment} detailBasePath="/teacher/assignments" reviewBasePath="/api/teacher/assignments" onReviewed={loadAssignment} />
    </div>
  );
}

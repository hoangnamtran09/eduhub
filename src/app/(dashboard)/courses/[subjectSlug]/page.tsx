...
export default function SubjectPage({ params }: { params: { subjectSlug: string } }) {
  ...
  const totalLessons = subject?.semesters.reduce((acc, semester) => 
    acc + semester.lessons.length, 0) || 0;

  const handleLessonClick = (lessonSlug: string) => {
    router.push(`/courses/${params.subjectSlug}/${lessonSlug}`);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 space-y-8">
            {subject.semesters.length > 0 ? (
              subject.semesters.map((semester, semesterIdx) => (
                <div key={semester.id} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {/* Single chapter for each semester */}
                    <div className="bg-white rounded-[28px] border border-slate-200/60 shadow-sm overflow-hidden group hover:border-brand-200 transition-all">
                      {/* Semester Header */}
                      <div className="px-6 py-4 flex items-center justify-between bg-slate-50/30 border-b border-slate-100">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="font-bold text-slate-900 text-base leading-tight">{semester.name}</h3>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                              Học kì {semesterIdx + 1} • {semester.lessons.length} bài học
                            </p>
                          </div>
                        </div>
                      </div>
